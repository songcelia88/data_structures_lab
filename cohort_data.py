...
def unique_houses(filename):
    ...
    with open(filename) as file:
        for line in file:
            line = line.rstrip()
            word = line.split("|") #tokenize the line
            house = word[2]
            if house != "":
                houses.add(house)
...
def sort_by_cohort(filename):
    ...
    with open(filename) as file:
        for line in file:
            ...
            if word[4] != "I" and word[4] != "G": #only going through students
                if word[4] == "Fall 2015":
...
def hogwarts_by_house(filename):
    ...
    # all_hogwarts = [house.sort() for house in all_hogwarts]
    return all_hogwarts
...
def find_cohort_by_student_name(student_list):
    ...
    # Code goes here
    student_lookup = input('Who are you looking for? ').title()
    for student in student_list:
        if student[0] == student_lookup:
            print('{} was in the {} cohort.'.format(student[0], student[3]))
            return student[3]
        else:
...
def find_house_members_by_student_name(student_list):
    ...
    # Code goes here
    student_lookup = input('Choose a student: ').title()
    for student in student_list:
        if student[0] == student_lookup:
            house = student[1]
            cohort = student[3]
            print('{} was in the house {} in the {} cohort.'.format(student[0], house, cohort))
            print('The following students are also in their house:')
            for peers in student_list:
                if peers[0] != student[0] and peers[1] == house and peers[3] == cohort:
                    print('{}'.format(peers[0]))
        # else:
        #     print('Student not found.')
    return
